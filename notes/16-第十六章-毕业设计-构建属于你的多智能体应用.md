...
你的毕业设计将以开源项目的形式提交到 Hello-Agents 的共创项目仓库（`Co-creation-projects`目录）。具体要求如下：
...
1. 提交方式：通过 GitHub 的 Pull Request（PR）提交

1. 评审流程：社区成员会 review 你的代码，提出改进建议，通过后合并到主仓库
...
# 检查
Git版本git --version# 配置
Git用户信息git config --global user.name "你的名字"git config --global user.email "你的邮箱"# 配置
GitHub SSH密钥（推荐）# 1. 生成
SSH密钥
ssh-keygen -t ed25519 -C "你的邮箱"# 2. 将公钥添加到
GitHub# 复制 ~/.
ssh/id_ed25519.pub 的内容# 在
GitHub Settings > SSH and GPG keys 中添加# 3. 测试连接
...
1. 访问 Hello-Agents 仓库：[https://github.com/datawhalechina/Hello-Agents](https%3A%2F%2Fgithub.com%2Fdatawhalechina%2FHello-Agents)

1. 点击右上角的"Fork"按钮，如图 16.1 红色方框位置

1. 选择你的 GitHub 账号，创建 Fork
...
```
# 如图16.2所示，克隆你
Fork的仓库git clone git@github.com:你的用户名/Hello-Agents.git
...
# 创建项目文件夹（格式:
GitHub用户名-项目名称）mkdir 你的用户名-项目名称
...
```
# 推送到你的
Fork仓库git push origin feature/你的项目名称
```
...
1. 点击"Pull requests"标签，如图 16.3 所示
...
```
git add .git commit -m "fix: 根据review意见修改XXX"git push origin feature/你的项目名称
```

1. 回复评论：在 GitHub 上回复 reviewer，说明你的修改
    ...
    def run(self, parameters: Dict[str, Any]) -> str:
        ...
        if not code:
            ...
            try:
                ...
                result = {"函数数量":
                len(functions),"类数量": len(classes),"代码行数": len(code.split('\n')),"函数列表": [f.name for f in functions],"类列表": [c.name for c in classes]}
                return str(result)
...
<text underline="true">**🎓 恭喜你完成了 Hello-Agents 教程的学习！🎉**</text> {align="center"}
