...
1. 提高质量：系统化的研究流程，避免遗漏重要信息

1. 可追溯：记录所有搜索结果和来源，方便验证和引用

1. 可扩展：可以轻松添加新的搜索引擎、数据源和分析工具
...
1. 前端发送：前端通过 SSE 连接到`/research/stream`

1. 后端接收：FastAPI 接收请求，创建研究状态

1. 规划阶段：调用研究规划 Agent，分解为 3 个子任务
...
1. 报告阶段：调用报告生成 Agent，整合所有总结

1. 流式返回：通过 SSE 推送进度和结果到前端

1. 前端展示：前端实时更新任务状态、进度条、日志、报告
...
```
python --version  # 应该显示 
Python 3.10.x 或更高node --version    # 应该显示 
v16.x.x 或更高npm --version     # 应该显示 8.
x.x 或更高
```
...
# 4. 编辑.
env文件，填入你的API密钥# 使用你喜欢的编辑器打开.
env文件# 至少需要配置：# - 
LLM_PROVIDER（如 openai、deepseek、qwen）# - 
LLM_API_KEY（你的LLM API密钥）# - 
SEARCH_API（如 duckduckgo、tavily）# 5. 启动后端
python src/main.py
```
...
```python
search_results = search_tool.run({"input": task.query,"backend": "tavily","mode": "structured","max_results": 5})
```

1. 获取搜索结果：提取标题、URL、摘要
...
1. 记录总结和来源：保存到 NoteTool


```
note_tool.run({"action": "create","title": task.title,"content": f"## {
task.title}\n\n{summary}\n\n## 来源\
n{sources}","tags": ["research", "summary"]})
```
...
Datawhale是一个专注于数据科学与AI领域的开源组织，成立于2018年[1]。组织的核心使命是"
for the learner，和学习者一起成长"，致力于构建一个纯粹的学习社区[2]。
...
class PlanningService:
    ...
    def _extract_tasks(self, response: str) -> List[dict]:"""从Agent响应中提取JSON"""
    # 使用正则表达式提取
    JSON部分
...
class ReportingService:
    ...
    self,
    research_topic: str,
    task_summaries: List[Tuple[TodoItem, str]]) -> str:
    # 格式化子任务总结
...
class ToolAwareSimpleAgent(SimpleAgent):
    ...
    def _execute_tool_call(self, tool_name: str, parameters: str) -> str:
        ...
        if self._tool_call_listener:
            self._tool_call_listener({"agent_name": self.name,"tool_name": tool_name,"parsed_parameters": parsed_parameters,"result": result,})
            return result
```
...
class DeepResearchAgent:
    def __init__(self, config: Configuration):
        ...
        # 创建工具调用监听器
    def tool_listener(call_info):
        self._emit_event({"type": "tool_call","agent": call_info["agent_name"],"tool": call_info["tool_name"],"parameters": call_info["parsed_parameters"],})
        # 创建三个
        Agent，都使用相同的监听器
...
1. 明确的输入输出：每个 Agent 的输入来自上一个 Agent 的输出

1. 无并发：同一时间只有一个 Agent 在工作
...
```python
#
config.pyclass SearchAPI(str, Enum):
TAVILY = "tavily"
...
# .
env
SEARCH_API=tavilyCopy to clipboardErrorCopied
```
...
```python
def deduplicate_sources(sources: List[dict]) -> List[dict]:"""去除重复的URL"""
seen_urls = set()
unique_sources = []
...
def limit_source_tokens(source: dict, max_tokens: int = 2000) -> dict:
    ...
    # 简单的
    Token估算：1个Token约等于4个字符
    max_chars = max_tokens * 4if len(snippet) > max_chars:
...
class NotesService:
    ...
    # 创建笔记
    self.note_tool.run({"action": "create","title": f"任务{task.id}：{task.title}","content": content,"tags": ["research", "summary"]})
...
1. 解析指令：`ToolRegistry`解析指令，提取工具名称和参数

1. 查找工具：`ToolRegistry`根据工具名称查找对应的工具

1. 调用工具：调用工具的`run`方法，传入参数

1. 返回结果：工具返回执行结果

1. 格式化结果：将结果格式化为字符串，返回给 Agent
...
1. 调用规划 Agent：调用 TODO Planner Agent 生成子任务列表

1. 解析 JSON 响应：从 Agent 的响应中提取 JSON 格式的子任务列表

1. 验证子任务格式**：确保每个子任务包含必需的字段（title、intent、query）
...
class PlanningService:
    ...
    def plan_todo_list(self, state: SummaryState) -> List[TodoItem]:"""规划TODO列表
...
1. 格式错误：JSON 可能缺少引号、逗号等

1. 字段缺失：某些子任务可能缺少必需字段

解决方案：

1. 使用正则表达式：提取 JSON 部分

1. 多种解析策略：先尝试提取 JSON 数组，再尝试直接解析

1. 字段验证：确保每个子任务包含必需字段
...
1. 逻辑清晰：子任务之间有明确的逻辑关系

1. 查询精准：搜索查询能够准确找到相关资料
...
1. 构建总结 Prompt：根据任务信息和搜索结果构建 Prompt

1. 调用总结 Agent：调用 Task Summarizer Agent 生成总结
...
class SummarizationService:
    ...
    self,
    task: TodoItem,
    search_results: List[dict]) -> Tuple[str, List[str]]:"""总结任务
...
1. 构建报告 Prompt：根据研究主题和子任务总结构建 Prompt

1. 调用报告 Agent：调用 Report Writer Agent 生成最终报告

1. 整理引用：将所有来源引用整理到参考文献部分
...
class ReportingService:
    ...
    self,
    research_topic: str,
    task_summaries: List[Tuple[TodoItem, str, List[str]]]) -> str:
    """生成最终报告
...
1. 执行搜索：调用 SearchTool 执行搜索

1. 处理结果：去重、限制 Token、格式化
...
class SearchService:
    ...
    self,
    query: str,
    max_results: int = 5) -> List[dict]:"""执行搜索
    ...
    try:
        # 调用SearchTool
        raw_response = self.search_tool.run({"input": query,"backend": self.config.search_api.value,"mode": "structured","max_results": max_results
        })
        # 提取结果
    ...
    def _deduplicate_sources(self, sources: List[dict]) -> List[dict]:"""去除重复的URL"""
    seen_urls = set()
    unique_sources = []
    for source in sources:
        url = source.get("url", "")
    ...
    self,
    sources: List[dict],
    max_tokens_per_source: int = 2000) -> List[dict]:"""限制每个来源的Token数量"""
    limited_sources = []
    for source in sources:
        snippet = source.get("snippet", "")
        # 简单的
        Token估算：1个Token约等于4个字符
        max_chars = max_tokens_per_source * 4if len(snippet) > max_chars:
        snippet = snippet[:max_chars] + "..."
...
1. 选择引擎：根据配置选择搜索引擎（tavily、duckduckgo、perplexity 等）
...
`class SearchService:def init(self, config: Configuration):`

`        self.config = config

``        self.search_tool = SearchTool(backend="hybrid")# 缓存目录`

`        self.cache_dir = Path("./cache/search")`

`        self.cache_dir.mkdir(parents=True, exist_ok=True)def search(`
...
`        use_cache: bool = True) -> List[dict]:"""执行搜索（带缓存）"""# 生成缓存键`

`        cache_key = self._generate_cache_key(query, max_results)`

`        cache_file = self.cache_dir / f"{cache_key}.json"# 尝试从缓存读取if use_cache and cache_file.exists():`

`            logger.info(f"从缓存读取搜索结果：{query}")with open(cache_file, "r", encoding="utf-8") as f:return json.load(f)# 执行搜索`

`        results = self._execute_search(query, max_results)# 保存到缓存if use_cache and results:with open(cache_file, "w", encoding="utf-8") as f:`

`                json.dump(results, f, ensure_ascii=False, indent=2)return results

``def _generate_cache_key(self, query: str, max_results: int) -> str:"""生成缓存键"""# 使用查询和最大结果数生成MD5哈希`

`        content = f"{query}_{max_results}_{self.config.search_api.value}"return hashlib.md5(content.encode()).hexdigest()`Copy to clipboardErrorCopied
...
1. 清晰的层次：主页面和研究页面分离，层次清晰

1. 易于关闭：点击关闭按钮或按 ESC 键即可返回主页面
...
1. 进度区域：显示当前研究进度（规划、执行、报告）

1. 内容区域：显示研究结果（Markdown 格式）

1. 底部栏**：显示状态信息（如"研究中..."、"已完成"）

对应的 Vue 实现如下所示(ResearchModal.vue):


```python
<template><div v-if="isOpen" class="modal-overlay" @click.self="close"><div class="modal-container"><!-- 顶部栏 --><div class="modal-header"><h2>{{ researchTopic }}</h2><button @click="close" class="close-button"><svg><!-- 关闭图标 --></svg></button></div><!-- 进度区域 --><div class="progress-section"><div class="progress-bar"><div
class="progress-fill"
:style="{ width: progressPercentage + '%' }"></div></div><div class="progress-text">{{ progressText }}</div></div><!-- 内容区域 --><div class="content-section"><div v-if="isLoading" class="loading-spinner"><div class="spinner"></div><p>研究中，请稍候...</p></div><div v-else class="markdown-content" v-html="renderedMarkdown"></div></div><!-- 底部栏 --><div class="modal-footer"><span class="status-text">{{ statusText }}</span></div></div></div></template><script setup lang="ts">import { ref, computed, watch }
from 'vue'import { marked }
from 'marked'interface Props {isOpen: boolean
researchTopic: string
}const props = defineProps<Props>()const emit = defineEmits<{close: []}>()// 状态const isLoading = ref(true)const progressPercentage = ref(0)const progressText = ref('准备中...')const statusText = ref('研究中...')const markdownContent = ref('')// 渲染Markdownconst renderedMarkdown = computed(() => {return marked(markdownContent.value)})// 关闭模态框const close = () => {emit('close')}// 监听ESC键const handleKeydown = (e: KeyboardEvent) => {if (e.key === 'Escape') {close()}}// 挂载时添加键盘监听watch(() => props.isOpen, (isOpen) => {if (isOpen) {
document.addEventListener('keydown', handleKeydown)} else {
document.removeEventListener('keydown', handleKeydown)}})</script><style scoped>.modal-overlay {position:
fixed;top: 0;left: 0;width: 100vw;height: 100vh;background-color: rgba(0, 0, 0, 0.5);display: flex;justify-content: center;align-items: center;z-index: 1000;}
......
</style>
```
...
```
/* 平板设备 */@media (max-width: 768px) {.modal-container {width: 95vw;height: 95vh;}.modal-header,
  .progress-section,
  .content-section,
  .modal-footer {padding: 15px 20px;}}/* 手机设备 */@media (max-width: 480px) {.modal-container {width: 100vw;height: 100vh;border-radius: 0;}.modal-header h2 {font-size: 18px;}}
```
...
1. 服务器建立 SSE 连接：返回`text/event-stream`响应

1. 服务器推送进度：定期推送研究进度（规划、执行、报告）

1. 客户端接收进度：监听 SSE 事件，更新 UI

1. 研究完成：服务器推送最终报告，关闭连接
...
app = FastAPI()async def research_stream(topic: str) -> AsyncGenerator[str, None]:"""研究流式生成器
...
try:
    ...
    # 调用PlanningService
    todo_items = await planning_service.plan_todo_list(topic)yield f"data: {json.dumps({'type': 'plan', 'data': [item.dict()
    for item in todo_items]})}\n\n"
    # 2. 执行阶段
    task_summaries = []
    for idx, task in enumerate(todo_items, start=1):
        # 更新进度
        percentage = 10 + (idx / len(todo_items)) * 70yield f"data: {json.dumps({'type': 'progress', 'stage': 'executing', 'percentage': percentage, 'text': f'正在研究任务{idx}/{len(todo_items)}：{task.title}'})}\n\n"
        # 搜索
...
```python
<script setup lang="ts">import { useResearch }
from '@/composables/useResearch'const {
isLoading,
progressPercentage,
progressText,
markdownContent,
error,
startResearch
} = useResearch()const handleStartResearch = (topic: string) => {startResearch(topic)}</script>
```
...
1. 质量可靠：通过专门的 Agent 保证每个环节的质量

1. 易于调试：可以单独调试每个子任务

1. 可扩展性好：可以轻松添加新的子任务或修改现有子任务
...
1. Prompt 优化：可以为每个 Agent 定制专门的 Prompt

1. 易于维护：修改一个 Agent 不会影响其他 Agent

1. 质量保证：每个 Agent 都是该领域的"专家"
